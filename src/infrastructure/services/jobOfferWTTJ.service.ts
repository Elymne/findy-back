...
export interface JobOfferWTTJService {
    filter: (source: JobOfferWTTJ[], filters: TextFilter[], histories: JobOfferHistory[]) => Promise<{ jobOffersWTTJFiltered: JobOfferWTTJ[]; newHistories: JobOfferHistory[] }>
}

export const JobOfferWTTJServiceImpl: JobOfferWTTJService = {
    filter: async function (source: JobOfferWTTJ[], filters: TextFilter[], histories: JobOfferHistory[]): Promise<{ jobOffersWTTJFiltered: JobOfferWTTJ[]; newHistories: JobOfferHistory[] }> {
        ...
        const result = source.filter((elem) => {
            ...
            if (!elem.tags.includes("Alternance")) {
                newHistories.push({
                    id: uuid,
